jQuery(document).ready(function($){
  ...
  var refreshExplore = function(){
    ...
    var failedTimeout = setTimeout(function(){
      showError("<p>:( Det virke til at kort servicen er nede.</p>");
    }, 3000);        
    $.ajax({
  ...
  var updatePagination = function (options){
    ...
    if(options['totalItems'] > 1){
      $('.tablenav-pages .displaying-num').html(options['totalItems'] + ' kort');
    }
    else if(options['totalItems'] == 1){
      $('.tablenav-pages .displaying-num').html('1 kort');
    }
    else{        
      $('.tablenav-pages .displaying-num').html('0 kort');
    }
